...
function createMeeting(username, requestDates, requestAccounts) {
  ...
  .then( (scheduleForDay)=>{
    ...
    // (Step 2c) timeslot is booked on all requestAccounts
      ...
      let $date_range = Moment(requestDates.start).format(DATE_TEXT_FORMAT) + ' from ' + Moment(requestDates.start).format(TIME_FORMAT) + ' - ' + Moment(requestDates.end).format(TIME_FORMAT)
      let message = ":X: Oh no! *" + $date_range + "* is booked on all the accounts."
...
function cancelMeeting (flag, username, requestDates, requestAccounts) {
  ...
  .then( (scheduleForDay)=>{
    ...
    if ( _.isEmpty(scheduleForDay) ){
      let message = 'Oops, it looks like no one\'s booked anything on *' + Moment(day.start).format(DATE_TEXT_FORMAT) + '*'
      return Util.Message.Simple( message )
    } 
    ...
    if (toDelete.length < 0) {
      let message = 'Hm...You don\'t have any meetings to cancel on *' + Moment(day.start).format(DATE_TEXT_FORMAT) + '*'
      return Util.Message.Schedule( message, scheduleForDay)
    }
...
exports.process = function (response, username){
    ...
    // generic responses
      ...
      let message = "Sorry I don't understand the request. Type `/joinme help` for usage instructions." 
   		return Util.Message.Simple( message )
   	}
    else if (output == 'help') {
