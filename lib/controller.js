...
function createMeeting(username, requestDates, requestAccounts) {
  ...
  .then( (scheduleForDay)=>{
    ...
    // (Step 2d) has overlap, luckily, one of the alternate account is bookable
      ...
      let $date_range = Moment(requestDates.start).format(DATETIME_FORMAT) + ' - ' + Moment(requestDates.start).format(DATETIME_FORMAT)
      let message = "Oh no! " + $date_range + "is booked for _" + requestAccounts.join(' ') + '_';
      let message2 = "Fear not, _" + altAccounts[0] + "_ is available. Try booking again on that account";
    ...
    .then( (meetingsList)=>{
        let message = 'Your meeting has been booked on *'+ account + '* \n -'
...
function seeSchedule (usermame, requestDates, requestAccounts) {
    ...
    .then( (scheduleForDay)=>{
      let message = 'Here are the booked times for ' + Moment(day.start).format(DATE_FORMAT) 
