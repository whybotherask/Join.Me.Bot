...
function seeSchedule (usermame, requestDates, requestAccounts) {
    ...
    .then( (scheduleForDay)=>{
      ...
      if ( _.isEmpty(scheduleForDay) ) {
        let message = ':tada: No bookings on *' + Moment(day.start).format(DATE_TEXT_FORMAT) + '* for ' + requestAccounts.join(' ') + '. Book whatever your :heart: desires';
...
function findBooking (flag, scheduleForDay, username, requestDates, requestAccounts) {
  ...
  let compareType = (flag == 'only date') ? 'day' : 'second';  // if only date was supplied, compare only datel otherwise go down to seconds
...
function cancelMeeting (flag, username, requestDates, requestAccounts) {
  ...
  .then( (scheduleForDay)=>{
    ...
    // if there is only 1 booking, then remove the requested one
      ...
      .then( (removedEntry) => {
        let $date_range = Moment(removedEntry.start).format(DATE_TEXT_FORMAT) + ' from ' + Moment(removedEntry.start).format(TIME_FORMAT) + ' - ' + Moment(removedEntry.start).format(TIME_FORMAT)
        let message = ':white_check_mark: Your *' + $date_range + '* meeting has been removed.'
        return Util.Message.Simple( message )
      }) // end Data.remove().then()
    }
    // if there are more than one meeting that match, then ask the use
    else if (toDelete.length > 1) {
      let message = ':thinking_face: Oh, you have more than one meeting that day on ' + requestAccounts.join(' ') + '. Which would you like to cancel?'
  ...
  */
    ...
    // generic responses
      ...
      let message = [
        ...
        ":point_right: Remember, try to use specific times:   _Thr 08:30am_:white_check_mark:   _Early today_:X:"
      ].join('\n');
      return Util.Message.Simple( message );
    }
  ...
  if (type.includes('cancel')) {
    ...
    .then( (removedEntry) => {
      ...
      let message = {
        ...
        "attachments": [
            {
              "text"      : ':white_check_mark: Your meeting *' + $date_range + '* on _' + removedEntry.account + '_ has been cancelled.',
              "color"     : "#CCCCCC",
