...
function cancelMeeting (flag, username, requestDates, requestAccounts) {
  ...
  .then( (scheduleForDay)=>{
    ...
    if ( _.isEmpty(scheduleForDay) ){
      let message = 'Oops, it looks like no one\'s booked anything on *' + Moment(day.start).format(DATE_FORMAT) + '*'
      return Util.Message.Simple( message )
    } 
    ...
    if (toDelete.length < 0) {
      let message = 'Hm...You don\'t have any meetings to cancel on *' + Moment(day.start).format(DATE_FORMAT) + '*'
      return Util.Message.Schedule( message, scheduleForDay)
    }
