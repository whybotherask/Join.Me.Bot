...
function create (dates, user, account) {
	...
	.find({
		'$or': [
			{'start': {		// check if any startdate is in between the requested start and end time
				'$gt': Moment(dates.start).toDate(),		
				'$lt': Moment(dates.end).toDate()
			}},
			{ 'end': {		// check if any endate is in between the requested start and end time
				'$gt': Moment(dates.start).toDate(),
				'$lt': Moment(dates.end).toDate()
			}}
		]
		...
		if (meetings.length > 0) {
			...
			if (free_accounts.length > 0) {
				return 'Sorry, there are no meetings that match your requests. There is an opening at that time on ' + free_accounts[0];
			}
			return ;
		} else {
